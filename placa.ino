...
#define ID_MQTT  "arduino-01"     //id mqtt (para identificação de sessão)
...
const byte mac[] = { 0xDE, 0xED, 0xBA, 0xFE, 0xFE, 0x01 };
...
int vagasOcupadas = 0;
const int tamVaga = 12; // Número de vagas disponíveis
...
void imprimirVagas() {
  ...
  if (millis() - timeVagas > intervaloVagas) {
    ...
    int vagasTotal = vagasOcupadas + vagasDisponiveis;
    lcd.setCursor(0, 1);
    lcd.print("Tot: ");
    lcd.setCursor(5, 1);
    lcd.print(vagasTotal);
